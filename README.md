...
安装时会出现 **安装 EasyTier SD-WAN 组件** 复选框：

- 需要无公网 IP、NAT、宿舍网、校园网或朋友 P2P 联机时，建议勾选。
- 只使用云服务器开服，或本机已有公网 IPv6 时，可以暂时不勾。
- 即使安装时没勾选，后续也可以在 WarpHost 的 SD-WAN 区域安装。
...
发布者打包前需要放入 EasyTier 二进制。最终用户不需要自己准备 EasyTier：
...
1. 打开 WarpHost。
2. 进入 **本地机房**。
3. 如果安装时未安装 EasyTier，先点击 **安装 EasyTier**。
4. 输入网络名称和网络密钥。
5. 点击连接 SD-WAN。
6. 将虚拟 IP 和游戏端口发给玩家。
